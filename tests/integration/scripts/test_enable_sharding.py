...
def test_enable_sharding(
    mongo_host, arctic, mongo_server, user_library, user_library_name
):
    c = mongo_server.api
    ...
    assert (
        call("buildinfo", read_preference=Primary(), session=None)
        in admin.command.call_args_list
...
def test_enable_sharding_on_db_other_failure(
    mongo_host, arctic, mongo_server, user_library, user_library_name
):
    ...
    with pytest.raises(OperationFailure):
        with patch.object(c, "admin") as admin:
            with patch("arctic.auth.MongoClient", return_value=c):
                ...
                run_as_main(
                    mes.main, "--host", mongo_host, "--library", user_library_name
                )
