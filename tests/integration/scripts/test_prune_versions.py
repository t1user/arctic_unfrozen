...
def test_prune_versions_symbol(mongo_host, library, library_name):
    with patch(
        "arctic.scripts.arctic_prune_versions.prune_versions", autospec=True
    ) as prune_versions:
...
def test_prune_versions_full(mongo_host, library, library_name):
    ...
    run_as_main(
        mpv.main, "--host", mongo_host, "--library", library_name, "--keep-mins", 10
    )
    assert [x["version"] for x in library.list_versions("symbol")] == [3, 2, 1]
    # Prune older than 0 minutes, v1 deleted
    run_as_main(
        mpv.main, "--host", mongo_host, "--library", library_name, "--keep-mins", 0
    )
...
def test_fix_broken_snapshot_references(library):
    library.write("cherry", "blob")
    one_day_ago = (
        time.time() - (3600 * 24.0) - 10
    )  # make sure we are a few seconds before 24 hours
