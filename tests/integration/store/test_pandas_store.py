...
def test_save_read_pandas_dataframe2(library):
    df = DataFrame(
        data=[1, 2, 3], index=pd.date_range(start="1/1/2011", periods=3, freq="h")
    )
    library.write("pandas", df)
    saved_df = library.read("pandas").data
    assert np.all(df.values == saved_df.values)
...
def test_save_read_pandas_dataframe_strings(library):
    df = DataFrame(
        data=["a", "b", "c"], index=pd.date_range(start="1/1/2011", periods=3, freq="h")
    )
    library.write("pandas", df)
    saved_df = library.read("pandas").data
    assert np.all(df.values == saved_df.values)
...
def test_save_read_pandas_dataframe_empty_multiindex_and_no_columns(library):
    expected = read_csv(
        StringIO("""STRATEGY MAC INSTRUMENT CONTRACT"""), delimiter=" "
    ).set_index(["STRATEGY", "MAC", "INSTRUMENT", "CONTRACT"])
    library.write("pandas", expected)
    saved_df = library.read("pandas").data
    assert np.all(expected.values == saved_df.values)
...
def test_append_pandas_dataframe(library):
    ...
    df2 = DataFrame(
        data=[4, 5, 6], index=pd.date_range(start="2/1/2011", periods=3, freq="h")
    )
    library.write("pandas", df)
...
def test_dataframe_append_should_promote_string_column(library):
    ...
    df2 = DataFrame(
        data2,
        index=DatetimeIndex(
            np.array([dt(2013, 1, 3)]).astype("datetime64[ns]"), name="DATETIME"
        ),
    )
...
def test_daterange_large_DataFrame(library):
    ...
    assert_frame_equal_(
        df[df.index[0] : df.index[100]], result, check_names=False, check_freq=False
    )
    # second 100
    result = library.read(
    ...
    assert_frame_equal_(
        df[df.index[100] : df.index[200]], result, check_names=False, check_freq=False
    )
    # first row
    result = library.read("MYARR", date_range=DateRange(df.index[0], df.index[0])).data
    assert_frame_equal_(
        df[df.index[0] : df.index[0]], result, check_names=False, check_freq=False
    )
    # last 100
    result = library.read("MYARR", date_range=DateRange(df.index[-100])).data
    assert_frame_equal_(
        df[df.index[-100] :], result, check_names=False, check_freq=False
    )
    # last 200-100
    result = library.read(
    ...
    assert_frame_equal_(
        df[df.index[-200] : df.index[-100]], result, check_names=False, check_freq=False
    )
    # last row
    result = library.read(
        "MYARR", date_range=DateRange(df.index[-1], df.index[-1])
    ...
    assert_frame_equal_(
        df[df.index[-1] : df.index[-1]], result, check_names=False, check_freq=False
    )
    # beyond last row
    result = library.read(
    ...
    assert_frame_equal_(
        df[dt(2020, 1, 1) : dt(2031, 9, 1)], result, check_names=False, check_freq=False
    )
...
def test_daterange(library, df, assert_equal):
    ...
    assert_equal(
        df,
        library.read("MYARR", date_range=DateRange(df.index[0])).data,
        check_freq=False,
    )
    assert_equal(
        df,
        library.read("MYARR", date_range=DateRange(df.index[0], df.index[-1])).data,
        check_freq=False,
    )
    assert_equal(
        df, library.read("MYARR", date_range=DateRange()).data, check_freq=False
    )
    assert_equal(
        df[df.index[10] :],
...
def test_daterange_append(library):
    ...
    assert_frame_equal_(
        rows,
        library.read("MYARR", date_range=DateRange(rows.index[0])).data,
        check_freq=False,
    )
    ...
    assert_frame_equal_(
        rows1,
        library.read("MYARR", date_range=DateRange(rows1.index[0])).data,
        check_freq=False,
    )
...
def test_daterange_fails_with_timezone_start(library):
    df = read_csv(
        StringIO("2015-08-10 00:00:00,200005,1.0\n" "2015-08-11 00:00:00,200016,3.0"),
        parse_dates=[0],
        names=["date", "security_id", "value"],
    ).set_index(["date", "security_id"])
    library.write("MYARR", df)
