...
def test_store_item_read_preference(library_secondary):
    with (
        patch("arctic.arctic.ArcticLibraryBinding.check_quota"),
        patch.object(
            library_secondary._versions,
            "with_options",
            wraps=library_secondary._versions.with_options,
    ...
    read_preferences = [
        call.kwargs.get("read_preference")
        for call in versions_with_options.call_args_list
    ]
...
def test_append_corrupted_new_version(library, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        ...
        assert library.read(symbol).data["near"].iloc[-1] == 40.0
...
def test_write_metadata(library, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        ...
        with patch("arctic.arctic.logger.info") as info:
            ...
            assert_frame_equal_(
                library.read(symbol, as_of=1).data, mydf_a, check_freq=False
            )
...
def test_restore_version_followed_by_append(library, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        ...
        with patch("arctic.arctic.logger.info") as info:
            ...
            item = library.read(symbol)
            assert_frame_equal_(
                item.data, pd.concat([mydf_a, mydf_c]), check_freq=False
            )
...
def test_fwpointer_enabled_write_delete_keep_version_append(library):
    ...
    with FwPointersCtx(FwPointersCfg.DISABLED):
        library.delete(symbol)
        assert (
            len(list(library._versions.find({"symbol": symbol}))) == 3
        )  # maintain all versions due to snapshot
