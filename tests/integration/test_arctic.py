...
def test_indexes(arctic):
    ...
    test_chunk = {
        "_id_": {"key": [("_id", 1)], "ns": "arctic.library", "v": index_version},
...
def test_disable_cache_by_settings(arctic):
    ...
    with patch(
        "arctic.arctic.Arctic._list_libraries", return_value=[lib]
    ) as uncached_list_libraries:
        with patch(
            "arctic.arctic.Arctic._list_libraries_cached", return_value=[lib]
        ) as cached_list_libraries:
            arctic.list_libraries()
            uncached_list_libraries.assert_called()
    ...
    with patch(
        "arctic.arctic.Arctic._list_libraries", return_value=[lib]
    ) as uncached_list_libraries_e:
        with patch(
            "arctic.arctic.Arctic._list_libraries_cached", return_value=[lib]
        ) as cached_list_libraries_e:
            arctic.list_libraries()
