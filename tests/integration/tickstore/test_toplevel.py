...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            dt(2010, 2, 1),
            dt(2012, 4, 1),
            [FEED_2010_LEVEL1, FEED_2011_LEVEL1, FEED_2012_LEVEL1],
        ),
...
def test_should_return_libraries_for_the_given_daterange(
    ...
    libraries = toplevel_tickstore._get_library_metadata(
        DateRange(start=start, end=end)
    )
    assert libraries == expected


def test_should_raise_exceptions_if_no_libraries_are_found_in_the_date_range_when_reading_data(
    toplevel_tickstore,
):
    toplevel_tickstore._collection.insert_one(
        {
    ...
    with pytest.raises(NoDataFoundException) as e:
        toplevel_tickstore.read(
            "blah", DateRange(start=dt(2012, 1, 1), end=dt(2012, 3, 1))
        )
    assert "No underlying libraries exist for the given date range" in str(e.value)


def test_should_return_data_when_date_range_falls_in_a_single_underlying_library(
    ...
    arctic.initialize_library(
        "test_current.toplevel_tickstore", tickstore.TICK_STORE_TYPE
    )
    ...
    res = toplevel_tickstore.read(
        "blah", DateRange(start=dt(2010, 1, 1), end=dt(2010, 1, 6)), list("ABCD")
    )
...
def test_should_return_data_when_date_range_spans_libraries(toplevel_tickstore, arctic):
    ...
    toplevel_tickstore.add(
        DateRange(start=dt(2010, 1, 1), end=dt(2010, 12, 31, 23, 59, 59, 999000)),
        "FEED_2010.LEVEL1",
    )
    toplevel_tickstore.add(
        DateRange(start=dt(2011, 1, 1), end=dt(2011, 12, 31, 23, 59, 59, 999000)),
        "FEED_2011.LEVEL1",
    )
    dates = pd.date_range("20100101", periods=6, tz=mktz("Europe/London"))
    ...
    res = toplevel_tickstore.read(
        "blah", DateRange(start=dt(2010, 1, 2), end=dt(2011, 1, 4)), list("ABCD")
    )
    expected_df = pd.concat([df_10[1:], df_11[:4]])
    assert_frame_equal(
        expected_df, res.tz_convert(mktz("Europe/London")), check_index_type=False
    )
...
def test_should_add_underlying_library_where_another_library_exists_in_a_non_overlapping_daterange(
    ...
    assert set(
        [res["library_name"] for res in toplevel_tickstore._collection.find()]
...
def test_should_raise_exception_if_library_does_not_exist(toplevel_tickstore):
    with pytest.raises(LibraryNotFoundException) as e:
        ...
        assert toplevel_tickstore._collection.find_one(
            {"library_name": "FEED_2010.LEVEL1"}
        )
    assert "Library FEED_2010.LEVEL1 was not correctly initialized" in str(e.value)


def test_should_raise_exception_if_date_range_for_library_overlaps_with_existing_libraries(
    toplevel_tickstore, arctic
):
    toplevel_tickstore._collection.insert_one(
        {
    ...
    with pytest.raises(OverlappingDataException) as e:
        toplevel_tickstore.add(
            DateRange(start=dt(2010, 6, 1), end=dt(2010, 12, 31, 23, 59, 59, 999000)),
            "FEED_2010a.LEVEL1",
        )
        assert toplevel_tickstore._collection.find_one(
            {"library_name": "FEED_2010.LEVEL1"}
...
def test_should_successfully_do_a_roundtrip_write_and_read_spanning_multiple_underlying_libraries(
    ...
    res = toplevel_tickstore.read(
        "blah",
        DateRange(start=dt(2010, 12, 1), end=dt(2011, 2, 1)),
        columns=list("ABCD"),
    )
    ...
    assert_frame_equal(
        data[dt(2010, 12, 1) : dt(2010, 12, 31)], res.tz_convert(mktz("Europe/London"))
    )
    lib2011 = arctic["FEED_2011.LEVEL1"]
    res = lib2011.read(
        "blah",
        DateRange(start=dt(2011, 1, 1), end=dt(2011, 2, 1)),
        columns=list("ABCD"),
    )
    assert_frame_equal(
        data[dt(2011, 1, 1) : dt(2011, 2, 1)], res.tz_convert(mktz("Europe/London"))
    )
...
def test_should_list_symbols_from_the_underlying_library(
    toplevel_tickstore, arctic, start, end, startr, endr
):
    arctic.initialize_library("FEED_2010.LEVEL1", tickstore.TICK_STORE_TYPE)
    arctic.initialize_library("FEED_2011.LEVEL1", tickstore.TICK_STORE_TYPE)
    toplevel_tickstore.add(
    ...
    assert expected_symbols == toplevel_tickstore.list_symbols(
        DateRange(start=start, end=end)
    )
...
def test_should_add_underlying_libraries_when_intialized(arctic):
    ...
    results = {
        result["library_name"]: {"start": result["start"], "end": result["end"]}
        for result in cur
    }
    expected_results = {
        "FEED_2010.LEVEL1": {
            "start": dt(2010, 1, 1),
            "end": dt(2010, 12, 31, 23, 59, 59, 999000),
        },
        "FEED_2011.LEVEL1": {
            "start": dt(2011, 1, 1),
            "end": dt(2011, 12, 31, 23, 59, 59, 999000),
        },
    }
...
def test_should_write_top_level_with_list_of_dicts(arctic):
    ...
    expected = pd.DataFrame(
        np.arange(57, dtype=np.float64), index=dates, columns=list("a")
    )
    toplevel_tickstore.write("blah", data)
    res = toplevel_tickstore.read(
        "blah", DateRange(start=dt(2010, 12, 1), end=dt(2011, 2, 1)), columns=list("a")
    )
    assert_frame_equal(
        expected,
        res.tz_convert(mktz("Europe/London")),
...
def test_should_write_top_level_with_correct_timezone(arctic):
    ...
    data = [{"index": dates[i], "a": i} for i in range(len(dates))]
    expected = pd.DataFrame(
        np.arange(len(dates), dtype=np.float64),
    ...
    assert_frame_equal(expected, res, check_freq=False, check_index_type=False)
    lib2010 = arctic["FEED_2010.LEVEL1"]
    # Check that only one point was written into 2010 being 3am on 31st
    assert (
        len(lib2010.read("blah", DateRange(start=dt(2010, 12, 1), end=dt(2011, 1, 1))))
        == 1
    )
...
def test_get_libraries_no_data_raises_exception(toplevel_tickstore, arctic):
    date_range = DateRange(
        start=dt(2009, 1, 1), end=dt(2010, 12, 31, 23, 59, 59, 999000)
    )
    with pytest.raises(NoDataFoundException):
...
def test_get_libraries_no_data_raises_exception_tzinfo_given(
    ...
    date_range = DateRange(
        start=dt(2009, 1, 1, tzinfo=tzinfo),
...
def test_get_library_metadata(arctic):
    ...
    with pytest.raises(NoDataFoundException):
        toplevel_tickstore.read(
            symbol,
            DateRange(
                start=dt(2010, 1, 1, tzinfo=tzinfo), end=dt(2011, 1, 2, tzinfo=tzinfo)
            ),
            columns=None,
        )
