...
def test_main_minimal():
    with patch("arctic.scripts.arctic_create_user.logger", autospec=True) as logger, \
         ...
         patch("arctic.scripts.arctic_create_user.create_client", autospec=True) as create_client:
    ...
    assert database.command.call_args_list == [
        call("createUser", "user", pwd="asdf", roles=[{"role": "readWrite", "db": "arctic_user"}])
    ]
...
def test_main_with_db_read_only():
    with patch("arctic.scripts.arctic_create_user.create_client", autospec=True) as create_client:
        run_as_main(main, "--host", "some_host", "--db", "some_db", "jblackburn")

    database = create_client.return_value.__getitem__.return_value
    assert database.command.call_args_list == [
        call("createUser", "jblackburn", pwd=ANY, roles=[{"role": "read", "db": "some_db"}])
    ]
...
def test_main_with_db_write():
    ...
    database = create_client.return_value.__getitem__.return_value
    assert database.command.call_args_list == [
        call("createUser", "jblackburn", pwd=ANY, roles=[{"role": "readWrite", "db": "some_db"}])
    ]
