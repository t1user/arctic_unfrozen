...
from arctic.tickstore.tickstore import (
    TickStore,
    IMAGE_DOC,
    IMAGE,
    START,
...
def dt_from_ms(ms, tz):
    return dt.fromtimestamp(ms / 1000, mktz(tz))
...
def test_mongo_date_range_query():
    ...
    assert query == {
        "s": {
            "$gte": dt(2014, 1, 1, 12, 0, tzinfo=mktz("UTC")),
...
def test_tickstore_pandas_to_bucket_image():
    ...
    assert final_image == {
        "index": dt(2014, 1, 4, 0, 0, tzinfo=mktz(tz)),
        "A": 3,
        "B": 3.0,
        "C": "DESC",
        "D": 1,
