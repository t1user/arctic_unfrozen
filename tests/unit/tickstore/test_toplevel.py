...
def test_raise_exception_if_date_range_does_not_contain_start_date():
    ...
    assert "The date range {0} must contain a start and end date".format(dr) in str(
        e.value
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            dt(2010, 12, 31, tzinfo=mktz("America/New_York")),
        ),
        (
            dt(2009, 12, 31, tzinfo=mktz("America/New_York")),
            dt(2010, 12, 31, tzinfo=mktz("America/New_York")),
        ),
        (
            dt(2009, 12, 31, 21, 10, tzinfo=mktz("America/New_York")),
