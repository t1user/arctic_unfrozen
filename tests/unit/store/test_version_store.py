...
def test_get_version_allow_secondary_True():
    ...
    assert vs._read_metadata.call_args_list == [
        call("symbol", as_of=None, read_preference=sentinel.read_preference)
    ]
    assert vs._do_read.call_args_list == [
        call(
            "symbol",
            vs._read_metadata.return_value,
            None,
            date_range=None,
    ...
    versions.find_one.assert_called_once_with(
        {
            ...
            "_id": {
                "$lt": bson.ObjectId.from_datetime(
                    dt(2013, 4, 1, 9, 0, tzinfo=mktz("Europe/Paris")) + dtd(seconds=1)
                )
            },
        },
        sort=[("symbol", pymongo.DESCENDING), ("version", pymongo.DESCENDING)],
...
def test_initialize_library():
    ...
    assert enable_sharding.call_args_list == [
        call(arctic_lib.arctic, arctic_lib.get_name(), hashed=sentinel.hashed)
    ]
...
def test_ensure_index():
    ...
    assert vs._collection.snapshots.create_index.call_args_list == [
        call([("name", 1)], unique=True, background=True)
    ]
    assert vs._collection.versions.create_index.call_args_list == [
        ...
        call(
            [("symbol", 1), ("version", -1), ("metadata.deleted", 1)],
            background=True,
            name="versionstore_idx",
        ),
        call([("parent", 1)], background=True),
    ]
    assert vs._collection.version_nums.create_index.call_args_list == [
        call("symbol", unique=True, background=True)
    ]
...
def _create_mock_versionstore():
    ...
    vs._add_new_version_using_reference.side_effect = (
        lambda *args: VersionStore._add_new_version_using_reference(vs, *args)
    )
...
def test_write_metadata_with_previous_data():
    ...
    with (
        patch("arctic.store.version_store.bson.ObjectId") as mock_objId,
...
def test_append_error_clean_retry():
    ...
    vs = create_autospec(
        ...
        _versions=Mock(
            ...
            find_one=Mock(__name__="find_one", return_value=previous_version),
        ),
        _arctic_lib=create_autospec(
            ArcticLibraryBinding, arctic=create_autospec(Arctic, mongo_host="some_host")
        ),
