...
from ._version_store_utils import (
    cleanup,
    get_symbol_alive_shas,
    _get_symbol_pointer_cfgs,
)
...
AsOf = str | int | dt | None
Metadata = dict[str, Any]
VersionDoc = dict[str, Any]
VersionId = Any
...
def register_version(version: str, numerical: int) -> None:
    global ARCTIC_VERSION, ARCTIC_VERSION_NUMERICAL
    ARCTIC_VERSION = version
    ARCTIC_VERSION_NUMERICAL = numerical


def register_versioned_storage(storageClass: Any) -> Any:
    existing_instances = [
        i for i, v in enumerate(_TYPE_HANDLERS) if str(v.__class__) == str(storageClass)
    ]
...
class VersionStore(object):
    ...
    def initialize_library(
        ...
        if "strict_write_handler" in kwargs:
            arctic_lib.set_library_metadata(
                "STRICT_WRITE_HANDLER_MATCH", bool(kwargs.pop("strict_write_handler"))
            )
    ...
    @mongo_retry
    def _last_version_seqnum(self, symbol: str) -> int:
    ...
    def _ensure_index(self) -> None:
        ...
        collection.versions.create_index(
            [("parent", pymongo.ASCENDING)], background=True
        )
    ...
    def __init__(self, arctic_lib: Any) -> None:
        ...
        self._reset()
        self._with_strict_handler: bool | None = None

    @property
    def _with_strict_handler_match(self) -> bool:
        if self._with_strict_handler is None:
            strict_meta = self._arctic_lib.get_library_metadata(
                "STRICT_WRITE_HANDLER_MATCH"
            )
            self._with_strict_handler = bool(
                STRICT_WRITE_HANDLER_MATCH if strict_meta is None else strict_meta
            )
...
%s""" % (self.__class__.__name__, hex(id(self)), indent(str(self._arctic_lib), 4))
    ...
    def list_symbols(
        ...
        **kwargs: Any,
        ...
        if snapshot is not None:
            ...
            return cast(list[str], self._versions.find(query).distinct("symbol"))
        ...
        return sorted(
            [x["_id"] for x in self._versions.aggregate(pipeline, allowDiskUse=True)]
        )

    @mongo_retry
    def has_symbol(self, symbol: str, as_of: AsOf = None) -> bool:
        """
    ...
    def read_audit_log(
        self, symbol: str | list[str] | None = None, message: Any = None
    ) -> list[dict[str, Any]]:
        """
        ...
        def _pop_id(x: dict[str, Any]) -> dict[str, Any]:
            x.pop("_id")
            return x
                ...
                versions.append(
                    {
                        ...
                        "date": ms_to_datetime(
                            datetime_to_ms(version["_id"].generation_time)
                        ),
                        "snapshots": [
                            snapshots[s]
                            for s in version.get("parent", [])
                            if s in snapshots
                        ],
                    }
                )
    ...
    def _find_snapshots(self, parent_ids: list[VersionId]) -> list[str]:
        snapshots = []
        for p in parent_ids:
    ...
    def _read_handler(self, version: VersionDoc, symbol: str) -> Any:
        handler = None
    ...
    @staticmethod
    def handler_can_write_type(handler: Any, data: Any) -> bool:
        type_method = getattr(handler, "can_write_type", None)
        if callable(type_method):
            return cast(bool, type_method(data))
        return False

    def _write_handler(
        self, version: VersionDoc, symbol: str, data: Any, **kwargs: Any
    ) -> Any:
        handler = None
        for h in _TYPE_HANDLERS:
        ...
        **kwargs: Any,
        ...
        try:
            ...
            _version = self._read_metadata(
                symbol, as_of=as_of, read_preference=read_preference
            )
    ...
    @mongo_retry
    def get_info(self, symbol: str, as_of: AsOf = None) -> dict[str, Any]:
        """
        ...
        if handler and hasattr(handler, "get_info"):
            return cast(dict[str, Any], handler.get_info(version))
        return {}

    @staticmethod
    def handler_supports_read_option(handler: Any, option: str) -> bool:
    ...
    def get_arctic_version(self, symbol: str, as_of: AsOf = None) -> int:
        """
        ...
        """
        return cast(
            int, self._read_metadata(symbol, as_of=as_of).get("arctic_version", 0)
        )
    ...
    def _do_read(
        ...
        **kwargs: Any,
        ...
        data = handler.read(
            self._arctic_lib, version, symbol, from_version=from_version, **kwargs
        )
    ...
    def read_metadata(
        ...
        """
        _version = self._read_metadata(
            symbol, as_of=as_of, read_preference=self._read_preference(allow_secondary)
        )
        return VersionedItem(
            symbol=symbol,
            library=self._arctic_lib.get_name(),
    ...
    def _read_metadata(
        self, symbol: str, as_of: AsOf = None, read_preference: Any = None
    ) -> VersionDoc:
        ...
        if as_of is None:
            ...
            if snapshot:
                _version = versions_coll.find_one(
                    {"symbol": symbol, "parent": snapshot["_id"]}
                )
            ...
            _version = versions_coll.find_one(
                {
                    "symbol": symbol,
                    "_id": {
                        "$lt": bson.ObjectId.from_datetime(as_of + timedelta(seconds=1))
                    },
                },
        ...
        return cast(VersionDoc, _version)

    def _insert_version(self, version: VersionDoc) -> None:
        try:
    ...
    def append(
        ...
        **kwargs: Any,
        ...
        if (
            ...
            and previous_metadata.get("deleted", False) is True
        ):
            return self.write(
                symbol=symbol,
                data=data,
    ...
    def write(
        ...
        **kwargs: Any,
        ...
        previous_version = self._versions.find_one(
            {"symbol": symbol, "version": {"$lt": version["version"]}},
            sort=[("version", pymongo.DESCENDING)],
        )

        handler = self._write_handler(version, symbol, data, **kwargs)
        handler.write(
            self._arctic_lib, version, symbol, data, previous_version, **kwargs
        )
    ...
    def _add_new_version_using_reference(
        self,
        symbol: str,
        new_version: VersionDoc,
        reference_version: VersionDoc,
        prune_previous_version: bool,
    ) -> VersionedItem:
        ...
        if prune_previous_version and reference_version:
            self._prune_previous_versions(
                symbol, new_version_shas=new_version.get(FW_POINTERS_REFS_KEY)
            )
    ...
    def write_metadata(
        ...
        **kwargs: Any,
        ...
        version = {
            k: previous_version[k] for k in previous_version.keys() if k != "parent"
        ...
        return self._add_new_version_using_reference(
            symbol, version, previous_version, prune_previous_version
        )

    @mongo_retry
    def restore_version(
        self, symbol: str, as_of: AsOf, prune_previous_version: bool = True
    ) -> VersionedItem:
        """
    ...
    @mongo_retry
    def _find_prunable_version_ids(
        self, symbol: str, keep_mins: int
    ) -> dict[VersionId, tuple[list[Any], Any]]:
        """
        ...
        return {
            v["_id"]: (
                [bson.binary.Binary(x) for x in v.get(FW_POINTERS_REFS_KEY, [])],
                get_fwptr_config(v),
            )
            for v in cursor
        }
    ...
    @mongo_retry
    def _find_base_version_ids(
        self, symbol: str, version_ids: list[VersionId]
    ) -> list[VersionId]:
        """
    ...
    def _prune_previous_versions(
        ...
        symbol: str,
        keep_mins: int = 120,
        keep_version: VersionId = None,
        new_version_shas: list[Any] | None = None,
    ) -> None:
        """
        ...
        shas_to_delete = [
            sha
            for v in prunable_ids_to_shas.values()
            for sha in v[0]
            if sha not in new_version_shas
        ]
    ...
    @mongo_retry
    def _delete_version(
        self, symbol: str, version_num: int, do_cleanup: bool = True
    ) -> None:
        """
        ...
        if not version:
            logger.error(
                "Can't delete %s:%s as not found in DB" % (symbol, version_num)
            )
            return
        ...
        if version.get("parent", None):
            for parent in version["parent"]:
                ...
                logger.error(
                    "Can't delete: %s:%s as pointed to by snapshot: %s"
                    % (symbol, version["version"], snap_name)
                )
                return
        ...
        if do_cleanup:
            cleanup(
                ...
                [version["_id"]],
                self._versions,
                shas_to_delete=tuple(
                    bson.binary.Binary(s) for s in version.get(FW_POINTERS_REFS_KEY, [])
                ),
    ...
    def delete(self, symbol: str) -> None:
        ...
        """
        logger.info(
            "Deleting data item: %r from %r" % (symbol, self._arctic_lib.get_name())
        )
        # None is the magic sentinel value that indicates an item has been deleted.
        sentinel = self.write(
            symbol, None, prune_previous_version=False, metadata={"deleted": True}
        )
        ...
        snapped_version = self._versions.find_one(
            {"symbol": symbol, "metadata.deleted": {"$ne": True}}
        )
        if not snapped_version:
            self._delete_version(symbol, sentinel.version)
        assert not self.has_symbol(symbol)

    def _write_audit(self, user: str, message: str, changed_version: Any) -> None:
        """
    ...
    def snapshot(
        ...
        versions: dict[str, AsOf] | None = None,
    ) -> None:
        """
        ...
        if snapshot:
            raise DuplicateSnapshotException(
                "Snapshot '%s' already exists." % snap_name
            )
        ...
        skip_symbol_set = set() if skip_symbols is None else set(skip_symbols)

        if versions is None:
            versions = {sym: None for sym in set(self.list_symbols()) - skip_symbol_set}

        # Loop over, and snapshot all versions except those we've been asked to skip
        for sym, as_of in versions.items():
            try:
                version = self._read_metadata(
                    sym, read_preference=ReadPreference.PRIMARY, as_of=as_of
                )
                # Update the parents field of the version document
                mongo_retry(self._versions.update_one)(
                    {"_id": version["_id"]}, {"$addToSet": {"parent": snapshot["_id"]}}
                )
            except NoDataFoundException:
    ...
    def delete_snapshot(self, snap_name: str) -> None:
        ...
        self._versions.update_many(
            {"parent": snapshot["_id"]}, {"$pull": {"parent": snapshot["_id"]}}
        )
    ...
    def _fsck(self, dry_run: bool) -> None:
        """
    ...
    def _cleanup_unreachable_shas(self, dry_run: bool) -> None:
        lib = self
        ...
        for symbol in chunks_coll.distinct("symbol"):
            ...
            if FwPointersCfg.DISABLED not in all_symbol_pointers_cfgs:
                ...
                logger.info(
                    "Cleaning up {} SHAs for symbol {}".format(
                        len(unreachable_shas), symbol
                    )
                )
                if not dry_run:
                    # Be liberal with the generation time.
                    id_time_constraint = {
                        "$lt": bson.ObjectId.from_datetime(dt.now() - timedelta(days=1))
                    }
    ...
    def _cleanup_orphaned_chunks(self, dry_run: bool) -> None:
        """
        ...
        for symbol in chunks_coll.distinct("symbol"):
            ...
            versions = set(
                versions_coll.find(
                    {"symbol": symbol, "_id": parent_id_constraint}
                ).distinct("_id")
            )
            ...
            for x in leaked_versions:
                chunk_count = mongo_count(
                    chunks_coll, filter={"symbol": symbol, "parent": x}
                )
                logger.info(
                    "%s: Missing Version %s (%s) ; %s chunks ref'd"
                    % (symbol, x.generation_time, x, chunk_count)
                )
                if versions_coll.find_one({"symbol": symbol, "_id": x}) is not None:
                    raise Exception(
                        "Error: version (%s) is found for (%s), but shouldn't be!"
                        % (x, symbol)
                    )
    ...
    def _cleanup_orphaned_versions(self, dry_run: bool) -> None:
        """
        ...
        for x in leaked_snaps:
            ...
            logger.info(
                "Missing Snapshot %s (%s) ; %s versions ref'd"
                % (x.generation_time, x, ver_count)
            )
