...
def _attempt_update_unchanged(
    symbol: str,
    unchanged_segment_ids: list[dict[str, Any]],
    collection: Any,
...
def _update_fw_pointers(
    ...
    """
    version[FW_POINTERS_CONFIG_KEY] = (
        ARCTIC_FORWARD_POINTERS_CFG.name
    )  # get the str as enum is not BSON serializable
    ...
    if len(version_shas) != version["segment_count"]:
        previous_version_id = (
            previous_version["_id"] if previous_version is not None else None
        )
...
def _spec_fw_pointers_aware(
    ...
    if (
        FW_POINTERS_CONFIG_KEY not in version
        or version[FW_POINTERS_CONFIG_KEY] == FwPointersCfg.DISABLED.name
    ):
...
def _fw_pointers_convert_append_to_write(previous_version: dict[str, Any]) -> bool:
    ...
    return (
        prev_fw_config is FwPointersCfg.ENABLED
        and ARCTIC_FORWARD_POINTERS_CFG is not FwPointersCfg.ENABLED
    )
...
class NdarrayStore(object):
    ...
    def _index_range(
        self, version: Any, symbol: Any, from_version: Any = None, **kwargs: Any
    ) -> Any:
        """
    ...
    def append(
        ...
        if (self._dtype(previous_version["dtype"]).fields is None) != (
        ...
        if str(dtype) != previous_version[
            "dtype"
        ] or _fw_pointers_convert_append_to_write(previous_version):
            logger.debug(
                "Converting %s from %s to %s"
                % (symbol, previous_version["dtype"], str(dtype))
            )
            ...
            # will not be reached.
            old_arr = _resize_with_dtype(
                self._do_read(collection, previous_version, symbol), dtype
            )
            ...
            if CHECK_CORRUPTION_ON_APPEND and _fast_check_corruption(
                ...
                logging.warning(
                    ...
                    "Converting append to concat and rewrite".format(
                        symbol, previous_version["version"]
                    )
                )
                dirty_append = (
                    True  # force a concat and re-write (use new base version id)
                )

            self._do_append(
                collection, version, symbol, item, previous_version, dirty_append
            )
    ...
    def _do_append(
        ...
        if (
            not dirty_append
            and version["append_count"] < _APPEND_COUNT
            and version["append_size"] < _APPEND_SIZE
        ):
            ...
            if len(item) > 0:
                ...
                try:
                    ...
                    if ARCTIC_FORWARD_POINTERS_CFG is FwPointersCfg.DISABLED:
                        ...
                        collection.update_one(
                            {"symbol": symbol, "sha": sha}, set_spec, upsert=True
                        )
                        _update_fw_pointers(
                            collection,
                            symbol,
                ...
                logger.debug(
                    "Appended segment %d for parent %s"
    ...
    def _concat_and_rewrite(
        ...
        for segment in sorted(
            collection.find(
                spec, projection={"_id": 1, "segment": 1, "compressed": 1, "sha": 1}
            ),
        ...
        if (
            ...
            raise DataIntegrityException(
                ...
                % (
                    symbol,
                    previous_version["version"],
                    previous_version["segment_count"]
                    - previous_version["append_count"]
                    - 1,
                    len(unchanged_segments),
                )
            )
        ...
        old_arr = self._do_read(
            collection, previous_version, symbol, index_range=read_index_range
        )
        if len(item) == 0:
            ...
            logger.debug(
                "Rewrite and compress/chunk %s, np.concatenate %s to %s"
                % (symbol, item.dtype, old_arr.dtype)
            )
            self._do_write(
                collection,
                version,
        ...
        if unchanged_segments:
            if version.get(FW_POINTERS_CONFIG_KEY) != FwPointersCfg.ENABLED.name:
                _attempt_update_unchanged(
                    symbol, unchanged_segments, collection, version, previous_version
                )
            ...
            _update_fw_pointers(
                ...
                previous_version,
                is_append=False,
                shas_to_add=version.get(FW_POINTERS_REFS_KEY, [])
                + [s["sha"] for s in unchanged_segments],
            )
    ...
    def check_written(collection: Any, symbol: str, version: dict[str, Any]) -> None:
        ...
        if (
            version.get(FW_POINTERS_CONFIG_KEY) == FwPointersCfg.HYBRID.name
            and ARCTIC_FORWARD_POINTERS_RECONCILE
        ):
            seen_chunks_reverse_pointers = mongo_count(
                collection, filter={"symbol": symbol, "parent": parent_id}
            )
    ...
    def write(
        ...
        if previous_version:
            if (
                ...
                and self.checksum(item[: previous_version["up_to"]])
                == previous_version["sha"]
            ):
    ...
    def _do_write(
        ...
        if previous_version:
            symbol_all_previous_shas.update(
                ...
                for x in collection.find(
                    {"symbol": symbol}, projection={"sha": 1, "_id": 0}
                )
            )
        ...
        if (
            ...
            and "segment_index" in previous_version
        ):
        ...
        chunks = [
            (item[i * rows_per_chunk : (i + 1) * rows_per_chunk]).tobytes()
            for i in idxs
        ]
        ...
        for i, chunk in zip(idxs, compressed_chunks):
            segment: dict[str, Any] = {
                "data": Binary(chunk),
                "compressed": True,
                "segment": min((i + 1) * rows_per_chunk - 1, length - 1)
                + segment_offset,
            }
            ...
            if ARCTIC_FORWARD_POINTERS_CFG is FwPointersCfg.DISABLED:
                if sha not in symbol_all_previous_shas:
                    ...
                    bulk.append(
                        pymongo.UpdateOne(
                            segment_spec, {"$addToSet": {"parent": version["_id"]}}
                        )
                    )
            else:
        ...
        if bulk:
            try:
                ...
                logger.error(
                    "Bulk write failed with details: %s (Exception: %s)"
                    % (bwe.details, bwe)
                )
                raise
    ...
    def _segment_index(
        self, new_data: Any, existing_index: Any, start: int, new_segments: Any
    ) -> Any:
        """
