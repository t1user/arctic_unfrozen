...
def gen_one_minute_rows(n_rows, dense):
    ...
    for header, header_range in ONE_MIN_ATTRIBUTES.items():
        data[header] = gen_sparse_rows_for_range(
            n_rows, header_range[0], header_range[1], dense
        )
    ...
    for day in range(0, n_row // 120):
        timestamps.extend(
            list(
                rrule(
                    MINUTELY,
                    count=active_minutes_daily,
                    dtstart=dt(2005, 1, 1) + td(days=day),
                )
            )
        )
...
def parse_args():
    ...
    parser.add_argument(
        "-r", "--rounds", type=int, help="number of rounds to run benchmarks", default=2
    )
    parser.add_argument(
        "-a",
        "--appends",
        type=int,
        help="number of appends for each symbol",
        default=75,
    )
    parser.add_argument(
        "-n",
        "--ndim",
        type=int,
        help="dimension of dataframe = size * size",
        default=500,
    )
    parser.add_argument(
        "-e",
        "--dense",
        help="Use dense or sparse (70 ish Nans) data",
        action="store_true",
    )
    parser.add_argument(
        "-d", "--mongodb", help="Mongo db endpoint.", default="127.0.0.1"
    )
    parser.add_argument(
        "-y", "--symbols", type=int, help="Total number of symbols to use", default=5
    )
...
def main(args):
    ...
    for dsize in data_size:
        enabled_reads[dsize] = mean_timedelta(
            [
                data["rtime"]
                for data in measure[dsize]
                if data["fwd"] == FwPointersCfg.ENABLED
            ]
        )
        disabled_reads[dsize] = mean_timedelta(
            [
                data["rtime"]
                for data in measure[dsize]
                if data["fwd"] == FwPointersCfg.DISABLED
            ]
        )
