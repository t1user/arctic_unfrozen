...
def get_random_df(nrows, ncols):
    ret_df = pd.DataFrame(
        ...
        columns=[
            "".join([chr(random.randint(ord("A"), ord("Z"))) for _ in range(8)])
...
def bench_compression_comparison(
    n_chunks, df_length, append_mul, pool_size, pool_step, repeats, use_raw_lz4, use_HC
):
...
def print_results(
    n_threads, chunk_size, n_chunks, total_mb, measurements, compare=None
):
